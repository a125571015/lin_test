$(function() {
    ...
    function init() {
        ...
        $('#btn-export-school').on('click', function (e) {
            ...
            if(stime != '' && etime != ''){
                location.href='/admin/school/get-school-use-condition?t=0&stime='+stime+'&etime='+etime;
            }else if(stime == '' && etime == ''){
                location.href='/admin/school/get-school-use-condition?t=0';
            }else{
        ...
        $('#btn-export-schoolclass').on('click', function (e) {
            ...
            if(stime != '' && etime != ''){
                location.href='/admin/school/get-schoolclass-use-condition?t=0&stime='+stime+'&etime='+etime;
            }else if(stime == '' && etime == ''){
                location.href='/admin/school/get-schoolclass-use-condition?t=0';
            }else{
                alert('請設定完整的使用區間或不設定');
            }
        });
    ...
    function initObj() {
        newObj.tblist = $('#tbList').DataTable({
            ...
            'ajax': {
                'type': 'post',
                'url': '/admin/school/get-parent-school-list',
                'data': function (data) {
                    data.name = $('#text-name').val();
                    data.school_id = $('#text-school-number').val();
                    data.domain = $('#text-domain').val();
            ...
            'initComplete': function () {
                $('#tbList tbody').on('click', 'tr', function(e){
                    ...
                    location.href =  '/admin/school/parentschool_view_data?id=' + data[2];
                });


                $('#text-name').on('keypress', function (e) {
                    if (e.which == 13) {
                        newObj.tblist.draw();
                    }
                });

                $('#text-school-number').on('keypress', function (e) {
                    if (e.which == 13) {
                        newObj.tblist.draw();
                    }
                });

                $('#text-domain').on('keypress', function (e) {
                    if (e.which == 13) {
                        newObj.tblist.draw();
                    }
                });
            ...
            //==========請求數據end
            'aoColumns': [
                {'sTitle': '總機構名稱', 'name': 'name'}, //'bSortable': false 禁止此列排序
                {'sTitle': '顯示名稱', 'name': 'viewname'},
                {'sTitle': '總機構編號', 'name': 'id'},   //, 'sClass': 'center', sWidth: '50px'
                {'sTitle': '域名', 'name': 'schoolclass_count', 'bSortable': false},
                //{'sTitle': '班級數量', 'name': 'user_total', 'bSortable': false},
            ],
        });
