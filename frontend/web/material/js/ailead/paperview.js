$(function() {
    ...
    function init() {
        ...
        $('#btn-online').on('click', function(e) {
            e.preventDefault();
            location.href = '/admin/quizpaper/sendpaper?type=online&pid=' + bs.getUrlVar('id');
        });

        $('#btn-manual').on('click', function(e) {
            e.preventDefault();
            location.href = '/admin/quizpaper/sendpaper?type=manual&pid=' + bs.getUrlVar('id');
        });
        ...
        $('#btn-modify').on('click', function(e) {
            e.preventDefault();
            modifyQuizpaper(newObj.info,newObj.quizpaperid);
        });
        ...
        $('#score-explain').on('click',function(e){
            ...
            swal({
                title: '<h4 style="color:#dca80d">高中配分說明</h4>',
                text: content,
                showCloseButton: true,
    ...
    function getQuizpaper() {
        $.ajax({
            url: '/admin/exam/get-quizpaper?pid=' + bs.getUrlVar('id'),
            type: 'GET',
            success: function(res) {
                ...
                if (quizpaper.public_status==0) {
                    $('#btn-private').hide();
                }else{
                    $('#btn-public').hide();
                }
                newObj.grade_code=quizpaper.grade_code;
                if (newObj.grade_code=='EP') {
                    $('#btn-manual').parent().remove();
                    $('#btn-export').parent().remove();
                }
                if (bs.getNormalGrade(newObj.grade_code)!='H0') {
                    $('#score-explain').parent().remove();
                }
                newObj.type_str='';
                if (quizpaper.source==2) {
                    newObj.type_str='&type=print';
                    $('#btn-online').parent().remove();
                    $('#btn-manual').parent().remove();
                }
                if (quizpaper.check_editor==0){
                    $('#btn-modify').hide();
                    $('#btn-private').hide();
                    $('#btn-public').hide();
                    $('#btn-delete').hide();
                ...
                newObj.info=JSON.parse(quizpaper.info);
                newObj.quizpaperid=res.data.id;
            },
            error: bs.errorHandler
        });
    }
            ...
            success: function(res) {
                ...
                $.each(quizContents, function(key, item) {
                  ...
                  <source src="'+m3u8+'" type="video/mp4">\
                    ...
                    '<div class="m-b-30">\
                      '+quiz_qtype_html+'\
					  <i class="fa fa-question-circle-o fa-2x btn-ask-quiz cursor-pointer m-r-10" data-id="'+qid+'" style="float:right"></i>\
                      <h4>' + no + '.</h4>\
                      '+ qg_html +'\
                      <div id="imgQA-' + idx + '">\
    ...
    function getQuizFullInfo(paperdetail) {
        ...
        $.ajax({
            url: '/admin/exam/get-quiz-full-info',
            type: 'POST',
            data: dataObj,
            success: function(res) {
                var msg = res.message;
    ...
    function modifyQuizpaper(dataObj,quizpaperid){
        ...
        dataObj.source='modify';
        dataObj.page_mode='paperview';
        dataObj.quizpaper_id=quizpaperid;
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                location.href = '/admin/quizpaper/manualcreate?subject=' + dataObj.pre_subject + '&pre=' + pre_id + '&source=modify'+newObj.type_str;
            },
            error: bs.errorHandler
        });
    }
    ...
    function exportQuizpaper(myInterval) {
        ...
        var dataObj = {};
        dataObj.pidAry = [];
        var pid= parseInt(bs.getUrlVar('id'));
