$(function() {
    ...
    function init(){
        ...
        jQuery('#export-date-stime').datetimepicker({
            format: 'Y/m/d',
            timepicker: false
        });

        jQuery('#export-date-etime').datetimepicker({
            format: 'Y/m/d',
            timepicker: false
        });
        ...
        $('#btn-export').on('click',function (e){
            e.preventDefault();
            var stime = $("#export-date-stime").val();
            var etime = $("#export-date-etime").val();

            if(stime != '' && etime != ''){
                location.href='/admin/ask/export-student-ask?&stime='+stime+'&etime='+etime;
            }else if(stime == '' && etime == ''){
                location.href='/admin/ask/export-student-ask';
            }else{
    ...
    function initObj() {
        ...
        newObj.tblist=$('#tbList').DataTable({
            ...
            'type':'POST',
            'ajax': {
                'url': '/admin/ask/ask-teacher-list',
                'type':'POST',
                'data':function(data){
            ...
            'initComplete': function () {
                ...
                // $('#quiz_qa_text').on('keypress',function(e){
                //     if(!searchRestricted()){return false;}
                //     if (e.which==13) {
                //         newObj.tblist.draw();
                //     }
